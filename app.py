...
with st.sidebar:
    ...
    signal_only = st.checkbox(
        "Signal-only preview (no orders)",
        value=False,
        disabled=state.is_running,
        help="Records strategy signals in trade history but never sends proposal or buy requests to Deriv. Useful for validating entries before risking funds.",
    )
    if selected_account:
        if signal_only:
            st.info("Signal-only mode is active. The bot will stream ticks and evaluate signals but will not place any orders.")
        ...
        execution_mode = resolve_execution_mode(
            selected_account_type, real_execution_confirmed, signal_only
        )
