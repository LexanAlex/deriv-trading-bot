...
## 1. Prerequisites
- **Python 3.9+**
- A registered **Deriv application** with the Trading and Read scopes. In the
  Deriv dashboard, set its OAuth Redirect URL to the exact public URL where
  this Streamlit dashboard is served.
...
Create a `.env` file beside `config.py` containing your registered legacy
WebSocket application ID:

```env
DERIV_APP_ID=your_registered_app_id
```

Run the dashboard at exactly the URL registered in Deriv (including HTTPS,
host, port, path, and trailing slash if present). Click **Login with Deriv**;
after login Deriv returns `token1`, `acct1`, and `cur1`. The application stores
these only in Streamlit session state and removes them from the URL immediately.

The `token1` flow is the legacy Deriv WebSocket OAuth flow, which is compatible
with this bot's `wss://ws.derivws.com/websockets/v3` trading connection. It is
not interchangeable with Deriv's newer Authorization Code + PKCE OAuth 2.0
flow, whose bearer tokens must use the newer API/OTP WebSocket flow.
...
## 6. Embedding into a Website (Hostinger)
