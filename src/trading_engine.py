...
class TradingEngine:
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        if self.paper_trading:
            # Paper mode records the signal but never sends a proposal or buy
            # request to Deriv, even though the market-data session is live.
            self.state.update_trade_outcome(trade_id, "PAPER", 0.0)
            self.state.set_status(
                f"PAPER {signal} signal recorded | Stake: {stake:.2f} | Barrier: {barrier}"
            )
            logger.info("Paper trade recorded | Signal=%s | TradeID=%s", signal, trade_id)
