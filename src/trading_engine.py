...
def resolve_execution_mode(
    account_type: str, real_execution_confirmed: bool, signal_only: bool = False
) -> str:
    """Resolve the immutable execution policy for one engine session.

    Signal-only preview records signals but never sends proposal/buy.
...
class TradingEngine:
    ...
    def __init__(
        ...
        self.execution_mode = resolve_execution_mode(
            self.account_type, self.real_execution_confirmed, self.signal_only
        )
        self.state = state
        ...
        # Instance-level pre-fetch (avoids global race conditions)
        self._pre_fetched_proposal_id: Optional[str] = None
        self._pre_fetched_ask_price: float = 0.0
        self._pre_fetched_direction: Optional[str] = None
        self._pre_fetched_stake: float = 0.0
    ...
    async def run(self):
        ...
        elif self.execution_mode == "SIGNAL_ONLY":
            self.state.set_status("Connecting to Deriv API for signal-only preview (no orders will be sent)...")
        else:
        ...
        # Start background pre-fetch loop
        pre_fetch_task = asyncio.create_task(self._pre_fetch_loop())
        ...
        try:
            ...
            pre_fetch_task.cancel()
            try:
                await pre_fetch_task
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        # Signal-only preview: record the signal locally and never send proposal/buy.
        if self.execution_mode == "SIGNAL_ONLY":
            self.state.add_trade(
                TradeRecord(
                    ...
                    account_type=self.account_type,
                    error_message="Signal-only preview: no proposal or buy request was sent.",
                )
            )
            self._strategy.on_trade_executed()
            self.state.set_status(f"Signal-only preview: {signal} at {entry_price:.4f} (no order sent).")
            logger.info("Signal-only preview TradeID=%s %s", trade_id, signal)
        ...
        try:
            ...
            # Use pre-fetched proposal only if direction and stake still match.
            proposal_id = None
            ask_price = 0.0
            if (
                self._pre_fetched_proposal_id
                and self._pre_fetched_direction == signal
                and abs(self._pre_fetched_stake - stake) < 0.01
            ):
                proposal_id = self._pre_fetched_proposal_id
                ask_price = self._pre_fetched_ask_price
                logger.info("Using pre-fetched proposal %s for TradeID=%s.", proposal_id, trade_id)
            else:
                # Fallback to synchronous fetching if pre-fetch failed or was stale.
                proposal = None
                max_retries = 2
            ...
            # The websocket timeout is already aggressively tuned to 2 seconds.
            ...
            self._trade_in_progress = False
            # Clear pre-fetched proposal so it cannot be reused for a later signal
            self._pre_fetched_proposal_id = None
            self._pre_fetched_ask_price = 0.0
            self._pre_fetched_direction = None
            self._pre_fetched_stake = 0.0
    ...
    async def _pre_fetch_loop(self):
        """Background loop to pre-fetch proposals when a pullback is detected."""
        while not self.state.stop_requested:
            if self.execution_mode in ("BLOCKED", "SIGNAL_ONLY"):
                await asyncio.sleep(2.0)
                continue

            state = self._strategy.get_state()
            stage = state.get("pattern_stage")
            trend = state.get("trend_direction")

            # Clear stale pre-fetch if we left PULLBACK / MOMENTUM or trend flipped
            if self._pre_fetched_proposal_id and stage not in ("PULLBACK", "MOMENTUM"):
                self._pre_fetched_proposal_id = None
                self._pre_fetched_ask_price = 0.0
                self._pre_fetched_direction = None
                self._pre_fetched_stake = 0.0

            if stage == "PULLBACK" and trend in ("UP", "DOWN") and not self._pre_fetched_proposal_id:
                martingale_state = self.state.get_martingale_state()
                stake = martingale_state["stake"]
                direction = "BUY" if trend == "UP" else "SELL"
                contract_type = CONTRACT_TYPE_BUY if direction == "BUY" else CONTRACT_TYPE_SELL
                barrier = self.barrier_buy if direction == "BUY" else self.barrier_sell

                try:
                    if not self._client or not self._client.connected:
                        logger.warning("Connection dropped during pre-fetch loop. Waiting for reconnection...")
                    ...
                    if proposal and proposal.get("id"):
                        self._pre_fetched_proposal_id = proposal.get("id")
                        self._pre_fetched_ask_price = float(proposal.get("ask_price", stake))
                        self._pre_fetched_direction = direction
                        self._pre_fetched_stake = stake
                        logger.info(
                            "Pre-fetched proposal %s for %s stake %.2f barrier %s",
                            self._pre_fetched_proposal_id, direction, stake, barrier,
                        )
                except Exception as e:
                    logger.debug("Pre-fetch proposal failed: %s", e)
                    await asyncio.sleep(2.0)
                    continue

            await asyncio.sleep(0.5)  # Faster poll so pullback → pre-fetch is prompt
