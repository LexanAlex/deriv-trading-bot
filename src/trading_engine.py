...
Final hardened trading engine.

Key upgrades:
- Fresh-signal trade gate.
- Startup warm-up.
- Post-cooldown grace period.
- Version-aware strategy state pushing to reduce dashboard churn.
- Safe reconnect handling.
- Ambiguous buy reconciliation.
- Proposal prefetch loop.
- Safe contract monitoring.
"""
...
class TradingEngine:
    ...
    async def run(self):
        ...
        # Arm trade gate after MTF data exists.
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    if await self._client.connect():
                        ...
                        # Require a fresh signal after reconnect.
                        self._gate_ready_monotonic = None
                        self._last_signal_monotonic = 0.0
    ...
    def _trade_gate_allows_trade(self, now_mono: float) -> Tuple[bool, str]:
        ...
        if self._last_signal_monotonic < grace_deadline:
            ...
            return (
                False,
                f"waiting for a fresh post-cooldown signal ({remaining_grace:.1f}s)",
            )
    ...
    def _push_tick_and_strategy_state(self, price: float, epoch: float) -> None:
        """Push full strategy state only when meaningful strategy state changed."""
            ...
            if outcome == "UNKNOWN":
                ...
                if outcome == "WON":
                    ...
                    self.state.on_trade_loss(
                        self.martingale_multiplier,
                        self.max_martingale_steps,
                    )
