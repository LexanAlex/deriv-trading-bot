...
# Pre-fetch state for ultra-fast execution
_pre_fetched_proposal_id: Optional[str] = None
_pre_fetched_ask_price: float = 0.0
...
class TradingEngine:
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        """
        global _pre_fetched_proposal_id, _pre_fetched_ask_price
        if self._trade_in_progress:
        ...
        try:
            ...
            # Check if we have a pre-fetched proposal
            proposal_id = _pre_fetched_proposal_id
            ask_price = _pre_fetched_ask_price
            
            if not proposal_id:
                # Fallback to synchronous fetching if pre-fetch failed or wasn't triggered
                proposal = None
                max_retries = 2
            ...
            self._trade_in_progress = False
            # Clear pre-fetched proposal
            _pre_fetched_proposal_id = None
            _pre_fetched_ask_price = 0.0
    ...
    async def _pre_fetch_loop(self):
        ...
        while not self.state.stop_requested:
            ...
            # Check if strategy is in PULLBACK state
            state = self._strategy.get_state()
            if state.get("pattern_stage") == "PULLBACK" and not _pre_fetched_proposal_id:
                # Pre-fetch proposal for the current Martingale step's stake
                martingale_state = self.state.get_martingale_state()
                stake = martingale_state["stake"]

                try:
                    # Ensure we are connected before sending a proposal
                    if not self._client.connected:
                    ...
                    proposal = await self._client.get_proposal(
                        symbol=SYMBOL,
                        contract_type=CONTRACT_TYPE_BUY if state.get("trend_direction") == "UP" else CONTRACT_TYPE_SELL,
                        stake=stake,
                        duration=CONTRACT_DURATION,
                        duration_unit=CONTRACT_DURATION_UNIT,
                        barrier=BARRIER_BUY if state.get("trend_direction") == "UP" else BARRIER_SELL,
                        currency=self.account_currency,
                    )
                    if proposal and proposal.get("id"):
                        _pre_fetched_proposal_id = proposal.get("id")
                        _pre_fetched_ask_price = float(proposal.get("ask_price", stake))
                        logger.info("Pre-fetched proposal %s for stake %.2f", _pre_fetched_proposal_id, stake)
                except Exception as e:
                    logger.debug("Pre-fetch proposal failed: %s", e)
                    # If it failed, wait longer before retrying to avoid flooding
                    await asyncio.sleep(2.0)
                    continue

            await asyncio.sleep(1.0)  # Polling interval for pre-fetches
