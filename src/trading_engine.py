...
class TradingEngine:
    """Async trading engine that communicates with the UI via StateManager."""
    ...
    async def run(self):
        if not _GLOBAL_ENGINE_LOCK.acquire(blocking=False):
            self.state.set_error(
                "Another MomentumMaster engine is already running in this Streamlit server process."
            )
            self.state.set_status("Start blocked: existing engine active.")
        ...
        try:
            ...
            if not await self._startup_open_contract_check():
                return

            self._pre_fetch_task = asyncio.create_task(
                self._pre_fetch_loop(),
                name="deriv-proposal-prefetch",
            )
            ...
            logger.info("Subscribing to tick stream for %s...", SYMBOL)
            ...
            _GLOBAL_ENGINE_LOCK.release()

    async def _startup_open_contract_check(self) -> bool:
        """Block startup if there is already an open contract on the traded symbol."""
        try:
            contracts = await self._client.get_portfolio()
        except DerivAPIError as exc:
            logger.warning(
                "Startup portfolio check failed; continuing without open-contract reconciliation: %s",
                exc,
            )
            return True
        ...
        open_contracts = [
            ...
            if contract.get("underlying_symbol") == SYMBOL and not bool(contract.get("is_sold", False))
        ]

        if not open_contracts:
            return True

        ids = ", ".join(str(contract.get("contract_id")) for contract in open_contracts[:5])
        msg = (
            f"Open contract(s) already exist on {SYMBOL}: {ids}. "
            "Trading is blocked for safety. Wait for settlement or check the Deriv statement, "
            "then restart the bot."
        )

        logger.error(msg)
        self.state.set_error(msg)
        self.state.set_status("Stopped: open contract detected at startup.")
        self.state.set_running(False)
        return False
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        try:
            ...
            if self._trade_in_progress:
                ...
                self._strategy.consume_reject_event()
            ...
            reject_reason = self._strategy.consume_reject_event()
            if reject_reason:
                self.state.record_signal_rejected_strategy(reject_reason)

            if signal in ("BUY", "SELL"):
                self.state.record_signal_generated()
                ...
                self.state.record_signal_skipped_gate(gate_reason)
    ...
    async def _refresh_mtf(self):
        ...
        try:
            items = list(MTF_GRANULARITIES.items())
            results = await asyncio.gather(
                *(
                    self._client.get_candles(SYMBOL, granularity, MTF_CANDLE_COUNT)
                    for _, granularity in items
                ),
            ...
            for (tf_label, _), result in zip(items, results):
                if isinstance(result, Exception):
                    raise result
                candles_by_tf[tf_label] = result
                logger.debug("MTF %s: %s candles fetched.", tf_label, len(result))
            ...
            logger.info("MTF analysis complete. Bias: %s | TF: %s", bias_str, tf_biases)
    ...
    async def _execute_trade(self, signal: str, entry_price: float, signal_state: Optional[Dict[str, Any]]):
        if self._trade_in_progress:
        ...
        score = int(signal_state.get("last_signal_score", 0))
        score_breakdown = dict(signal_state.get("last_signal_score_breakdown", {}))
        entry_mode = str(signal_state.get("last_entry_mode") or "")
        trend_kind = str(signal_state.get("trend_kind") or "")
        trend_age = int(signal_state.get("trend_age", 0))
        micro_bias = str(signal_state.get("micro_bias") or "")
        mtf_bias = str(signal_state.get("mtf_bias") or "")
        mtf_agreement = int(signal_state.get("mtf_agreement", 0))
        ...
        if self.execution_mode == "BLOCKED":
            ...
            self.state.record_signal_skipped_gate("execution blocked")
        ...
        try:
            ...
            if cached and cached.matches(
                ...
                prefetch_used = True
                proposal_latency = max(0.0, time.monotonic() - cached.created_at)
                self.state.record_prefetch_hit()
                ...
                self.state.record_prefetch_miss()
                ...
                proposal_latency = time.monotonic() - proposal_start
                self.state.record_latency("proposal", proposal_latency)
            ...
            try:
                ...
                buy_latency = time.monotonic() - buy_start
                self.state.record_latency("buy", buy_latency)
            ...
            payout = float(buy_response.get("payout", 0.0))
            ...
            self.state.record_signal_executed()

            self.state.confirm_trade(
                trade_id=trade_id,
                contract_id=contract_id,
                payout=payout,
                proposal_latency=proposal_latency,
            ...
            logger.info(
                "Contract %s bought | Buy Price=%s | Payout=%s | TradeID=%s | "
                "Signal-to-fill latency: %.3fs",
                contract_id,
                buy_price,
                payout,
                trade_id,
            ...
            outcome, pnl, exit_reason, settlement_latency = await self._monitor_contract(
                contract_id,
                buy_price,
                payout,
            )

            self.state.record_latency("settlement", settlement_latency)
            ...
            if outcome == "UNKNOWN":
                ...
                if outcome == "WON":
                    logger.info("Trade WON | PnL=%.2f | TradeID=%s", pnl, trade_id)
                    self.state.on_trade_win()
                    self.state.set_status(f"Trade WON! P&L: +{pnl:.2f}")
                else:
                    logger.info("Trade LOST | PnL=%.2f | TradeID=%s", pnl, trade_id)
                    self.state.on_trade_loss(self.martingale_multiplier, self.max_martingale_steps)

                    new_state = self.state.get_martingale_state()
                    self.state.set_status(
                        f"Trade LOST. Next stake: {new_state['stake']:.2f} (Step {new_state['step']})"
                    )
            ...
            if e.code == "AMBIGUOUS_BUY":
                ...
                logger.critical("Ambiguous buy outcome for TradeID=%s: %s", trade_id, e)
    ...
    async def _reconcile_after_buy_timeout(
        ...
        logger.warning(
            "Buy receipt was not confirmed. Checking the live portfolio for a matching untracked fill before stopping."
        )
        ...
        for attempt in range(1, 4):
            ...
            if not self._client.connected:
                logger.warning("Reconnecting before portfolio reconciliation (%d/3).", attempt)
            ...
            if len(matches) > 1:
                logger.error(
                    "Portfolio reconciliation found multiple possible fills; automatic adoption would be unsafe."
                )
                return None

        logger.error(
            "No unique matching contract was found. The buy outcome remains ambiguous and trading must stay stopped "
            "until the account statement is checked."
        )
    ...
    async def _monitor_contract(
        ...
        while time.time() - start_time < max_wait:
            if not self._client.connected:
                logger.warning("Connection lost while monitoring contract %s; reconnecting.", contract_id)
            ...
            try:
                ...
                if is_expired or is_sold or status_name in {"won", "lost", "sold"}:
                    ...
                    settlement_latency = time.monotonic() - start_mono

                    if status_name == "won" or profit > 0:
                        return "WON", round(profit, 2), "TOUCH", settlement_latency

                    return "LOST", round(profit, 2), "EXPIRED", settlement_latency
        ...
        logger.warning("Contract %s monitoring timed out without a confirmed outcome.", contract_id)
