...
BUG-7: API token is taken raw from a Streamlit text_input widget and passed
       directly to DerivAPIClient without stripping whitespace. Fixed by
       stripping the token before use.

--- REWRITE NOTES (execution-path fix, round 2) ---
The previous version kept "pre-fetch" fresh by firing a brand-new one-shot
proposal request on almost every tick while a pullback/momentum pattern was
active. That is what caused the reported timeout storm: on a 1-tick/second
symbol this meant a fresh proposal request roughly every ~0.8s, stacked on
top of the tick stream, the periodic MTF candle pulls, and the app-level
heartbeat, all sharing one WebSocket connection. Once Deriv (or anything in
front of it) fell even slightly behind, every one of those request types
started timing out together — proposal, MTF, heartbeat, buy — which is
exactly the log pattern reported, not a dead-connection problem.

The fix: stop polling for proposals altogether. Deriv's proposal endpoint
supports ``subscribe: 1`` the same way ticks do — send the request once and
Deriv pushes fresh ask_price updates on its own as the market moves. This
engine now opens exactly two such streams up front (one for BUY, one for
SELL) and keeps them open for as long as the bot runs, refreshing them only
when the Martingale stake actually changes (i.e. after a trade settles) —
not every tick. The latest pushed value for each direction is cached
in-memory (_proposal_cache) and used the instant a signal fires, with a
synchronous one-shot proposal+buy as a fallback if the cache is missing or
stale. This is also a direct implementation of the user's own request: a
proposal that's "already there" and ready to fire on, instead of the bot
re-asking Deriv for one on every tick.

Everything about the strategy (src/strategy.py) and the dashboard are
unchanged; only the execution path below was touched.
"""
...
@dataclass
class _StreamedProposal:
    """The latest value pushed by one of the two standing proposal
    subscriptions (BUY / SELL), cached so a signal can use it with zero
    network round trips."""

    proposal_id: Optional[str] = None
    ask_price: float = 0.0
    stake: float = 0.0
    received_at: float = 0.0  # time.monotonic()

    def is_usable(self, stake: float, max_age: float) -> bool:
        if not self.proposal_id:
            return False
        if abs(self.stake - stake) > max(0.01, stake * 0.02):
            return False
        if (time.monotonic() - self.received_at) > max_age:
            return False
        return True
...
class TradingEngine:
    ...
    # A streamed proposal is refreshed by Deriv roughly once per underlying
    # tick, so on this 1-tick/second symbol it should almost never be older
    # than this when a signal fires. If it is (e.g. a brief stall right after
    # (re)subscribing), the engine falls back to a synchronous fetch rather
    # than trade on a stale price.
    PREFETCH_MAX_AGE_FOR_USE = 3.0

    # Minimum time between (re)arm *attempts* for a given direction. Without
    # this, a transient failure (e.g. TIMEOUT while arming) would otherwise
    # be retried on literally every tick from _ensure_proposal_streams_armed,
    # which recreates the exact request-storm this design is meant to avoid.
    # force=True call sites (after connect/reconnect/trade settlement) bypass
    # this — those are already naturally infrequent.
    PROPOSAL_ARM_RETRY_INTERVAL = 5.0

    # Deriv error codes that mean "the connection/request was transient" —
    # worth retrying. Anything else (e.g. a schema rejection) is treated as
    # permanent for this session; see _arm_proposal_streams().
    _TRANSIENT_ARM_ERROR_CODES = {"TIMEOUT", "CONNECTION_LOST", "NOT_CONNECTED", "CONNECTION_CLOSED"}
    ...
    def __init__(
        ...
        # Standing proposal streams, one per direction, kept open for the
        # whole session instead of being fetched per tick. Keyed the same
        # way as signals: "BUY" / "SELL".
        self._proposal_cache: Dict[str, _StreamedProposal] = {
            "BUY": _StreamedProposal(),
            "SELL": _StreamedProposal(),
        }
        self._proposal_stake_armed: Dict[str, float] = {"BUY": 0.0, "SELL": 0.0}
        self._proposal_last_arm_attempt: Dict[str, float] = {"BUY": 0.0, "SELL": 0.0}

        # Flipped to False the first time Deriv rejects a subscribed
        # proposal request for a non-transient reason (e.g. a schema/
        # validation error). Some accounts/App IDs are on an Options API
        # variant that doesn't support "proposal"+"subscribe" the way the
        # standard public API does — in that case retrying is pointless, so
        # the engine stops trying for the rest of this run and relies
        # entirely on the synchronous one-shot proposal+buy fallback in
        # _execute_trade instead.
        self._proposal_streaming_supported: bool = True
    ...
    async def run(self):
        ...
        try:
            ...
            # Open the two standing proposal streams (BUY/SELL) so a
            # ready-to-use proposal is available the instant a signal fires,
            # without any per-tick polling. Best-effort: if this fails here,
            # _execute_trade() still has the synchronous fallback path.
            await self._arm_proposal_streams()
    ...
    async def _reconnect(self, max_attempts: int = 5) -> bool:
        ...
        """
        # Any cached proposal is tied to the connection that streamed it.
        # Once we reconnect, its subscription is gone from Deriv's side too,
        # so drop the cached values rather than risk buying against a
        # proposal ID Deriv no longer recognises.
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                if await self._client.connect():
                    await self._client.subscribe_ticks(SYMBOL, self._on_tick)
                    await self._arm_proposal_streams(force=True)
    ...
    async def _shutdown(self):
        ...
        if self._client:
            await self._client.forget_all_proposals()
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # No per-tick proposal fetching anymore: the BUY/SELL proposal
            # streams opened in run()/_reconnect() are kept fresh by Deriv
            # itself. We only need to make sure they're actually armed (e.g.
            # first tick after connecting, or the stake changed).
            await self._ensure_proposal_streams_armed()
            ...
            if signal in ("BUY", "SELL"):
                ...
                # Nothing to do here anymore — both directions' proposals are
                # already streaming continuously regardless of pattern stage,
                # so there's no separate "start fetching now" action needed.
                logger.debug("Pre-fetch signal received; proposal streams are already live.")
    ...
    # ------------------------------------------------------------------
    # Standing proposal streams (replaces the old per-tick pre-fetch)
    # ------------------------------------------------------------------

    def _invalidate_proposal_cache(self) -> None:
        self._proposal_cache = {"BUY": _StreamedProposal(), "SELL": _StreamedProposal()}
        self._proposal_stake_armed = {"BUY": 0.0, "SELL": 0.0}

    def _make_proposal_callback(self, direction: str):
        """Returns a callback that stores the latest streamed proposal for
        `direction` ("BUY"/"SELL") — O(1), no I/O, just a cache update."""

        def _on_proposal(proposal: Dict[str, Any]) -> None:
            proposal_id = proposal.get("id")
            if not proposal_id:
                return
            self._proposal_cache[direction] = _StreamedProposal(
                proposal_id=proposal_id,
                ask_price=float(proposal.get("ask_price", 0.0) or 0.0),
                stake=self._proposal_stake_armed.get(direction, 0.0),
                received_at=time.monotonic(),
            )

        return _on_proposal

    async def _arm_proposal_streams(self, force: bool = False) -> None:
        """(Re)open both BUY and SELL proposal streams at the current
        Martingale stake. Safe to call repeatedly — subscribe_proposal()
        replaces any existing subscription under the same tag.

        Two safeguards against turning a failure into a hot loop:
          - If Deriv rejects the request for a non-transient reason (e.g. a
            schema/validation error — this account's proposal endpoint
            simply doesn't accept "subscribe: 1" with these fields),
            streaming is permanently disabled for the rest of this run.
            Retrying a rejection like that changes nothing except how much
            noise it generates.
          - Transient failures (timeouts, connection issues) are still
            retried, but not more than once per PROPOSAL_ARM_RETRY_INTERVAL
            per direction, so a slow patch of network can't turn into a
            per-tick request storm on its own.
        """
        if self.execution_mode == "BLOCKED" or self._client is None:
            return
        if not self._client.connected:
            return
        if not self._proposal_streaming_supported:
            return  # Permanently disabled this session; fallback handles it.

        stake = self.state.get_martingale_state()["stake"]
        directions = {
            "BUY": (CONTRACT_TYPE_BUY, self.barrier_buy),
            "SELL": (CONTRACT_TYPE_SELL, self.barrier_sell),
        }
        now = time.monotonic()
        for direction, (contract_type, barrier) in directions.items():
            already_armed = (
                not force
                and abs(self._proposal_stake_armed.get(direction, -1.0) - stake) <= max(0.01, stake * 0.02)
                and self._proposal_cache[direction].proposal_id
            )
            if already_armed:
                continue
            if not force and (now - self._proposal_last_arm_attempt.get(direction, 0.0)) < self.PROPOSAL_ARM_RETRY_INTERVAL:
                continue  # Back off; a previous attempt failed recently.

            self._proposal_last_arm_attempt[direction] = now
            try:
                self._proposal_stake_armed[direction] = stake
                await self._client.subscribe_proposal(
                    tag=direction,
                    symbol=SYMBOL,
                    ...
                    barrier=barrier,
                    currency=self.account_currency,
                    callback=self._make_proposal_callback(direction),
                )
                logger.info("Armed %s proposal stream (stake %.2f, barrier %s).", direction, stake, barrier)
            except DerivAPIError as exc:
                if exc.code not in self._TRANSIENT_ARM_ERROR_CODES:
                    # A permanent rejection (e.g. InputValidationFailed): this
                    # account/App ID's proposal endpoint does not accept a
                    # subscribed proposal with the fields this bot sends.
                    # Retrying will never succeed, so stop for the session.
                    self._proposal_streaming_supported = False
                    logger.error(
                        "Deriv rejected the streamed-proposal request as invalid: %s. "
                        "This account's Options API may not support 'subscribe: 1' on "
                        "proposals the way the standard Deriv API does. Disabling proposal "
                        "streaming for the rest of this run — trades will use a fresh "
                        "synchronous proposal request instead (slightly slower per trade, "
                        "but reliable). Check with your API/App ID provider for the correct "
                        "subscribed-proposal request shape if you want streaming enabled.",
                        exc,
                    )
                    return  # Stop attempting the other direction too.
                logger.warning("Could not arm %s proposal stream (will retry in %.0fs): %s", direction, self.PROPOSAL_ARM_RETRY_INTERVAL, exc)

    async def _ensure_proposal_streams_armed(self) -> None:
        """Cheap per-tick check: only actually talks to Deriv if the
        Martingale stake changed or a stream isn't armed yet, and only if
        streaming hasn't been permanently disabled (see _arm_proposal_streams)."""
        if self.execution_mode == "BLOCKED" or self._client is None or not self._client.connected:
            return
        if not self._proposal_streaming_supported:
            return
        stake = self.state.get_martingale_state()["stake"]
        needs_arming = any(
            abs(self._proposal_stake_armed.get(direction, -1.0) - stake) > max(0.01, stake * 0.02)
            or not self._proposal_cache[direction].proposal_id
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        """
        Execute a contract trade based on the given signal.

        Workflow:
          1. Get current Martingale stake.
          2. Use the live-streamed proposal for this direction if it's fresh
             and matches the current stake, else request a price proposal
             from the API synchronously.
        ...
        try:
            ...
            cached = self._proposal_cache.get(signal)
            if cached and cached.is_usable(stake, self.PREFETCH_MAX_AGE_FOR_USE):
                proposal_id = cached.proposal_id
                ask_price = cached.ask_price
                logger.info(
                    "Using streamed proposal %s for TradeID=%s (age %.2fs) — no proposal round trip needed.",
                    proposal_id, trade_id, time.monotonic() - cached.received_at,
                )
            else:
                logger.info(
                    "Streamed proposal for TradeID=%s was stale, missing, or mismatched; fetching fresh.",
                    trade_id,
                )
                proposal = None
                max_retries = 2
                for attempt in range(max_retries + 1):
                    try:
                        logger.info("Requesting Deriv proposal (attempt %d) for TradeID=%s.", attempt + 1, trade_id)
                        ...
                        if "TIMEOUT" in str(e) and attempt < max_retries:
                            logger.warning("Proposal timeout on attempt %d, retrying immediately...", attempt + 1)
                            continue
                        raise e  # Re-raise if not a timeout or last attempt

                if not proposal:
                    raise DerivAPIError("Failed to get proposal after retries.", "RETRY_EXHAUSTED")
            ...
            # Submit the buy request IMMEDIATELY. No delays, no logging pauses.
            ...
            self._trade_in_progress = False
            # The Martingale stake may have just changed (win resets it, loss
            # multiplies it) — re-arm both proposal streams at the new stake
            # so the next signal, in either direction, again has a ready
            # proposal waiting instead of stale/mismatched cached values.
            await self._arm_proposal_streams(force=True)
