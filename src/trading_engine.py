...
The pre-fetch mechanism used to be a single pair of *module-level* globals
(_pre_fetched_proposal_id / _pre_fetched_ask_price), refreshed by a
background loop polling on a flat 1-second timer, completely decoupled from
the tick stream that actually drives the strategy. That caused three
concrete problems, all fixed below:

  1. Module-level globals are shared across every TradingEngine instance in
     the process. Two sessions (or a stale one left running) could clobber
     each other's pre-fetched proposal. Now stored per-instance.
  2. The old loop pre-fetched exactly once per pullback (guarded by
     `if ... and not _pre_fetched_proposal_id`) and never refreshed it while
     the pullback deepened or momentum ticks accumulated. On a 1-tick-per-
     second symbol, a proposal's ask_price is frequently stale by the time
     BUY/SELL actually fires 2+ ticks later. Now the proposal is refreshed
     on every tick while the pattern is active, and staleness/mismatch are
     checked again immediately before use, with a synchronous fallback if
     the cached one doesn't qualify.
  3. The old loop pre-fetched using the raw config.BARRIER_BUY /
     config.BARRIER_SELL constants instead of self.barrier_buy /
     self.barrier_sell, silently ignoring any custom barrier passed into the
     engine's constructor. Fixed to use the instance's configured barriers,
     matching what _execute_trade actually sends to Deriv.
"""
...
class _PrefetchedProposal:
    """A cached Deriv proposal, tagged with what it's good for and when it
    was fetched, so it can be validated for freshness/match right before use
    ...
    def is_usable(self, contract_type: str, barrier: str, stake: float, max_age: float) -> bool:
        if not self.proposal_id:
            return False
        if self.contract_type != contract_type or self.barrier != barrier:
            return False
        if abs(self.stake - stake) > max(0.01, stake * 0.02):
            return False
        if (time.monotonic() - self.fetched_at) > max_age:
            return False
        return True
...
class TradingEngine:
    ...
    # A proposal's ask_price is only trustworthy for a moment on a
    # 1-tick-per-second symbol. If the cached pre-fetch is older than this
    # when the signal fires, it's discarded in favour of a fresh synchronous
    # proposal + buy rather than risking a rejected/mispriced order.
    PREFETCH_MAX_AGE_FOR_USE = 1.2

    # How long a cached proposal is treated as "good enough to not bother
    # re-fetching yet" while the pattern is still active. Kept just under
    # one tick interval so the tick-driven refresh below effectively
    # attempts a new fetch every tick once the previous one has landed.
    PREFETCH_REFRESH_INTERVAL = 0.8
    ...
    def __init__(
        ...
        # Instance-scoped pre-fetch state (was module-level global before).
        self._prefetch = _PrefetchedProposal()
    ...
    async def _reconnect(self, max_attempts: int = 5) -> bool:
        ...
        """
        # Any pre-fetched proposal is tied to the connection that fetched
        # it. Once we reconnect, treat it as gone rather than risk buying
        # against a proposal ID Deriv no longer recognises.
    ...
    async def _on_tick(self, tick_data: Dict[str, Any]):
        ...
        try:
            ...
            # Tick-driven pre-fetch refresh. Runs on every tick while a
            # pullback/momentum pattern is active so the cached proposal
            # never gets more than ~1 tick stale, instead of a detached
            # 1-second poll loop racing the tick stream.
            self._maybe_refresh_prefetch(strategy_state)
            ...
            if signal in ("BUY", "SELL"):
                ...
                logger.debug("Pre-fetch signal received; tick-driven refresh will fetch a proposal.")
    ...
    def _cancel_prefetch_task(self) -> None:
        if self._prefetch_task and not self._prefetch_task.done():
            self._prefetch_task.cancel()
        self._prefetch_task = None

    def _maybe_refresh_prefetch(self, strategy_state: Dict[str, Any]) -> None:
        """Called on every tick. Keeps at most one pre-fetch in flight and
        re-fetches whenever the cached proposal is missing, stale, or no
        longer matches the current trend direction/stake."""
        if self.execution_mode == "BLOCKED" or self._client is None:
            return

        stage = strategy_state.get("pattern_stage")
        if stage not in ("PULLBACK", "MOMENTUM"):
            # No active setup right now — don't hold on to (or keep paying
            # for) a proposal that has nothing to be used for.
            if self._prefetch.proposal_id or self._prefetch_task:
                self._invalidate_prefetch()
            return
        ...
        contract_type = CONTRACT_TYPE_BUY if trend == "UP" else CONTRACT_TYPE_SELL
        barrier = self.barrier_buy if trend == "UP" else self.barrier_sell
        stake = self.state.get_martingale_state()["stake"]

        matches = (
            self._prefetch.contract_type == contract_type
            and self._prefetch.barrier == barrier
            and abs(self._prefetch.stake - stake) <= max(0.01, stake * 0.02)
        )
        fresh_enough = (time.monotonic() - self._prefetch.fetched_at) <= self.PREFETCH_REFRESH_INTERVAL

        if matches and fresh_enough:
            return  # Cached proposal is good enough for now.
        if self._prefetch_task and not self._prefetch_task.done():
            return  # A fetch is already in flight; let it land first.
        if not self._client.connected:
            return  # Nothing we can do until the main loop reconnects.

        self._prefetch_task = asyncio.create_task(
            self._fetch_prefetch_proposal(contract_type, barrier, stake)
        )

    async def _fetch_prefetch_proposal(self, contract_type: str, barrier: str, stake: float) -> None:
        try:
            ...
            if proposal_id:
                ...
                logger.debug(
                    "Pre-fetched %s proposal %s (stake %.2f, barrier %s).",
                    contract_type, proposal_id, stake, barrier,
                )
        except DerivAPIError as exc:
            logger.debug("Pre-fetch proposal attempt failed (will retry next tick): %s", exc)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.debug("Unexpected pre-fetch error.", exc_info=True)
    ...
    async def _execute_trade(self, signal: str, entry_price: float):
        ...
        Workflow:
          ...
          2. Use the pre-fetched proposal if it's fresh and matches, else
             request a price proposal from the API synchronously.
        ...
        # Stop the background pre-fetch loop from competing with the buy
        # request for the socket while a trade is actually being placed.
        self._cancel_prefetch_task()
        ...
        # by watching the buy request time out later — by which point it's
        # ambiguous whether Deriv ever saw it.
        if not self._client.connected:
        ...
        try:
            # Step 1: Use the pre-fetched proposal only if it's fresh and
            # matches this exact signal's contract type/barrier/stake.
            # Otherwise fall back to a synchronous fetch so we never buy
            # against a stale or mismatched price.
            ...
            if self._prefetch.is_usable(contract_type, barrier, stake, self.PREFETCH_MAX_AGE_FOR_USE):
                proposal_id = self._prefetch.proposal_id
                ask_price = self._prefetch.ask_price
                logger.info(
                    "Using pre-fetched proposal %s for TradeID=%s (age %.2fs).",
                    proposal_id, trade_id, time.monotonic() - self._prefetch.fetched_at,
                )
            else:
                if self._prefetch.proposal_id:
                    logger.info(
                        "Pre-fetched proposal for TradeID=%s was stale or mismatched; fetching fresh.",
                        trade_id,
                    )
                proposal = None
                max_retries = 2
