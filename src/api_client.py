...
class DerivAPIClient:
    ...
    # A 5-tick contract expires in ~5 seconds. If Deriv takes >3s to answer a
    # proposal/buy request, the entry price is stale and the trade execution
    # is likely too late. We use 3s to balance speed with real-world network
    # latency between Streamlit Cloud and Deriv servers.
    REQUEST_TIMEOUT = 3.0
    
    # Pings are the fastest way to detect a dead connection. A 5s interval
    # ensures we know within a few seconds if the WebSocket dropped, rather
    # than waiting for a 20s TCP keepalive.
