...
class DerivAPIClient:
    ...
    async def authorize(self, token: Optional[str] = None) -> bool:
        ...
        Works identically for a manually-generated Personal Access Token
        (PAT) and for an OAuth2 session token (the `token1` value Deriv
        appends to the redirect URL after a successful OAuth login) — Deriv's
        `authorize` API call treats both the same way.

        Args:
            token: Optional token to authorize with. If omitted, falls back
                   to the token passed to the constructor (self.api_token).
                   Passing it explicitly lets a caller (re)authorize with a
                   freshly-obtained OAuth token without recreating the client.
        """
        auth_token = (token if token is not None else self.api_token)
        auth_token = (auth_token or "").strip()
        if not auth_token:
            logger.error("No API token available for authorization.")
            self._is_authorized = False
            return False

        # Keep self.api_token in sync so reconnect()/connect() re-authorize
        # with the same (possibly OAuth-supplied) token.
        self.api_token = auth_token

        try:
            response = await self._send_request({"authorize": self.api_token})
