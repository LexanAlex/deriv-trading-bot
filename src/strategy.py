...
Final v5.2 — Institutional Tick Momentum Engine

Design goals:
- Take only high-quality trend trades.
- Reject choppy, weak, whipsaw-heavy, or exhausted moves.
- Keep dashboard refresh calm by only bumping state version when
  meaningful public strategy state changes.
- Keep the exact public interface used by TradingEngine:
...
class StrategyEngine:
    """Final v5.2 institutional-quality tick momentum strategy."""
    ...
    def __init__(
        ...
        # State versioning for a calm dashboard.
        # The engine only pushes full strategy state when this version changes.
    ...
    def _sync_state_version(self) -> None:
        """Bump state version only when dashboard-relevant state changes.

        trend_age and micro_bias are intentionally excluded from the signature:
        they can change too often and cause unnecessary dashboard churn.
        They are still included in get_state() whenever state is pushed.
        """
        signature = (
    ...
    def _reject_signal(
        ...
        # Reset the pattern so the next signal must be earned from a fresh setup.
