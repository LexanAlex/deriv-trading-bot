...
# Minimum absolute net displacement (index points) for a window to count as a
# trend. Filters efficient but tiny micro-moves that cannot realistically
# reach a ±0.08 barrier within 5 ticks on Volatility 10 (1s).
MIN_TREND_DISPLACEMENT = 0.04
...
class StrategyEngine:
    ...
    def _scan_windows(
        ...
        efficiency ratio clears `threshold` and net displacement is large
        enough. Returns (direction, window) or None."""
        if len(ticks) < window_min:
            return None
        for window in range(min(window_max, len(ticks)), window_min - 1, -1):
            sample = ticks[-window:]
            net = abs(sample[-1] - sample[0])
            if net < MIN_TREND_DISPLACEMENT:
                continue
