...
def start_bot(api_token: str, app_id: str, account_id: str, account_currency: str,
              ...
              barrier_sell: str, strategy_sensitivity: str):
...
with st.sidebar:
    ...
    st.caption(
        "Every trade the bot opens is sent to Deriv as a real order on "
        "whichever account is selected above — demo or real."
    )
...
if selected_account:
    ...
    mode_line = f"LIVE ORDERS — executing on your {account_type} account"
else:
    account_line = "Awaiting a secure Deriv account connection"
    mode_line = "LIVE ORDERS — select an account to begin"
st.markdown(
