...
with st.sidebar:
    ...
    if oauth_token:
        ...
        # link_button opens a new tab. target=_top is intentional: Streamlit
        # can be rendered in an iframe, and target=_self would then navigate
        # only that iframe instead of the browser URL bar.
        st.markdown(
            f'<a href="{oauth_login_url}" target="_top" '
            'style="display:block;text-align:center;background:#ff444f;color:white;'
            'padding:0.55rem 1rem;border-radius:0.45rem;text-decoration:none;'
            'font-weight:600;">Login with Deriv</a>',
            unsafe_allow_html=True,
        )
