...
async def _get_authorized_account(token: str) -> dict:
    """Fetch account identity/balance after Deriv returns an OAuth token."""
    client = DerivAPIClient(token, DERIV_APP_ID)
    try:
        if not await client.connect():
            return {}
        return client.authorized_account
    finally:
...
if callback_token:
    ...
    try:
        ...
        # The token is still retained for the trading engine, which will show
        # its own authorization error if Deriv rejects it.
...
with st.sidebar:
    ...
    if oauth_token:
        ...
        # Deriv explicitly blocks framed pages. Use the browser's supported
        # top-level handoff; the returned tab is the authorized bot session.
        st.link_button(
            "Login with Deriv",
            oauth_login_url,
            type="primary",
        ...
        st.caption("Complete login in the opened tab. It returns to this bot and displays the authorized account and balance.")
