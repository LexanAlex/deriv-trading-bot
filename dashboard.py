...
with st.sidebar:
    ...
    if oauth_token:
        ...
        # A plain browser anchor keeps OAuth in this same browser tab. Unlike
        # st.link_button it does not create a second Streamlit session, so the
        # callback query parameters return to and authorize this dashboard.
        st.markdown(
            f'<a href="{oauth_login_url}" target="_self" rel="noopener" '
            'style="display:block;box-sizing:border-box;width:100%;text-align:center;'
            'background:#ff4b4b;color:#ffffff;padding:0.62rem 1rem;'
        ...
        st.caption("Deriv opens in this tab and must return to this exact Streamlit URL after login.")
