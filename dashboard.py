...
async def _get_authorized_account(token: str) -> dict:
    """Validate a returned legacy OAuth token and read account/balance once."""
    websocket_url = f"wss://ws.derivws.com/websockets/v3?l=EN&app_id={DERIV_APP_ID}"
    async with websockets.connect(websocket_url, open_timeout=15, close_timeout=5) as socket:
        await socket.send(json.dumps({"authorize": token}))
        response = json.loads(await asyncio.wait_for(socket.recv(), timeout=15))
    if "error" in response:
        message = response["error"].get("message", "Deriv rejected the login token.")
...
if callback_token:
    ...
    try:
        account = asyncio.run(_get_authorized_account(callback_token))
        if not account:
            raise RuntimeError("Deriv did not return account information.")
        st.session_state.deriv_account_id = account.get("loginid", st.session_state.deriv_account_id)
        st.session_state.deriv_account_currency = account.get("currency", st.session_state.deriv_account_currency)
        st.session_state.deriv_account_balance = account.get("balance")
        st.session_state.deriv_oauth_error = ""
    except Exception as exc:
        # Keep no token when the callback token cannot be validated.
        st.session_state.pop("deriv_oauth_token", None)
        st.session_state.deriv_account_balance = None
        st.session_state.deriv_oauth_error = f"Deriv login could not be verified: {exc}"
...
with st.sidebar:
    ...
    callback_error = st.session_state.get("deriv_oauth_error", "")
    if oauth_error:
    ...
    if oauth_token:
        ...
        balance = st.session_state.get("deriv_account_balance")
        if balance is not None:
            st.metric("Deriv balance", f"{account_currency} {float(balance):,.2f}")
        if st.button("Log out", use_container_width=True):
            for key in ("deriv_oauth_token", "deriv_account_id", "deriv_account_currency", "deriv_account_balance"):
        ...
        # Deriv blocks framed authentication. The returned browser tab becomes
        # the authorized Streamlit bot session and shows the account below.
        st.link_button(
            "Login with Deriv",
            oauth_login_url,
            type="primary",
        ...
        st.caption("Complete login in the opened Deriv tab. It returns to this bot and displays your account and balance.")
