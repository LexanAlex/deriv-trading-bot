...
with st.sidebar:
    ...
    max_martingale_steps = st.slider(
        ...
        DEFAULT_MAX_MARTINGALE_STEPS,
        disabled=state.is_running,
        help="Total stake levels including the initial stake.",
    )
    ...
    st.caption(
        f"Stake progression: {stake_preview} | Total exposure: {total_exposure:.2f} | "
        "Total levels including initial stake."
    )
    ...
    st.caption(
        ...
        f"**Contract:** Touch | {CONTRACT_DURATION} {CONTRACT_DURATION_UNIT}\n\n"
        "**MTF:** 5m / 15m / 30m"
    )
...
st.markdown(
    ...
    <div class="terminal-header">
        <div>
            ...
            <div class="terminal-subtitle">Quality Pullback Momentum | {SYMBOL_DISPLAY}</div>
        </div>
        <div style="text-align:right;">
            ...
            <span style="color:#6e7681;font-size:0.68rem;">
                {display_account_type} | {display_account_id[:8] if display_account_id else "—"} | {display_currency}
            </span>
        </div>
    </div>
    """,
...
with col_tc:
    ...
    if stats["total_trades"] > 0:
        ...
        caption_parts = [
            ...
            f"Stake: {stake_label} {display_currency}",
        ]
...
with col_state_panel:
    ...
    if current_price:
        st.markdown(
            f'<div class="state-value" style="font-size:1.4rem;">{current_price:.4f}</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f'<div class="state-label">Trend</div><div class="state-value {trend_cls}">{trend}</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        f'<div class="state-value {mtf_cls}">{mtf} '
        f'<span style="font-size:0.75rem;color:#6e7681;">({mtf_agreement}/3)</span></div>',
        unsafe_allow_html=True,
    )
    ...
    last_tick_local_time = heartbeat.get("last_tick_local_time")

    if last_tick_local_time is not None:
        age = time.time() - last_tick_local_time
...
with st.expander("Diagnostics", expanded=False):
    ...
    dcol1.metric("Tick Rate", f"{diag['tick_rate_per_min']}/min")
    dcol2.metric("Signals", diag["signals_generated"])
    dcol3.metric("Executed", diag["signals_executed"])
    dcol4.metric("Skipped", diag["signals_skipped_gate"])

    dcol5, dcol6, dcol7, dcol8 = st.columns(4)

    dcol5.metric("Rejected", diag["signals_rejected_strategy"])
    dcol6.metric("Prefetch Hit %", f"{diag['prefetch_hit_rate']:.1f}%")
    dcol7.metric("Reconnects", diag["reconnects"])
    dcol8.metric(
        "Buy Latency",
        f"{diag['buy_latency']['avg']:.3f}s" if diag["buy_latency"]["count"] else "—",
    )

    st.caption(
        f"Last reject: {diag['last_reject_reason'] or '—'} | "
        f"Last skip: {diag['last_skip_reason'] or '—'}"
    )

    st.json(
        {
            "reject_reason_counts": diag["reject_reason_counts"],
            "skip_reason_counts": diag["skip_reason_counts"],
            "proposal_latency": diag["proposal_latency"],
            "buy_latency": diag["buy_latency"],
            "settlement_latency": diag["settlement_latency"],
        }
    )
        ...
        records.append(
            {
                ...
                "Score": t.score if t.score else "",
                "Mode": t.entry_mode or "",
